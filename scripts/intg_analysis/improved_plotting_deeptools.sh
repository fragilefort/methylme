...
# Liver figures use only Liver regions and Liver signal tracks.
# Kidney figures use only Kidney regions and Kidney signal tracks.
# This script creates NEW results and does not overwrite the combined analysis.
...
plot_matrix() {
    local matrix="$1" outdir="$2" prefix="$3" title="$4" cmap="$5" ylabel="$6"
    shift 6
    plotHeatmap --matrixFile "$matrix" --outFileName "$outdir/${prefix}.png" \
        --outFileNameMatrix "$outdir/${prefix}_values.tab" --sortRegions descend --sortUsing mean \
        --colorMap "$cmap" --zMin auto --zMax auto --heatmapWidth 14 --heatmapHeight 16 \
        --dpi 300 --interpolationMethod nearest --boxAroundHeatmaps no \
        --whatToShow "plot, heatmap and colorbar" --legendLocation upper-right \
        --labelRotation 0 --yAxisLabel "$ylabel" --plotTitle "$title" "$@"
    plotHeatmap --matrixFile "$matrix" --outFileName "$outdir/${prefix}.pdf" \
        --sortRegions descend --sortUsing mean --colorMap "$cmap" --zMin auto --zMax auto \
        --heatmapWidth 14 --heatmapHeight 16 --dpi 300 --interpolationMethod nearest \
        --boxAroundHeatmaps no --whatToShow "plot, heatmap and colorbar" \
        --legendLocation upper-right --labelRotation 0 --yAxisLabel "$ylabel" \
        --plotTitle "$title" "$@"
}

run_promoter() {
    local signal="$1" outdir="$2" prefix="$3" title="$4" cmap="$5" ylabel="$6"
    mkdir -p "$outdir"
    computeMatrix reference-point --referencePoint TSS -b 500 -a 1500 --binSize 25 -p "$THREADS" \
        -R "$GENES" -S "$signal" --samplesLabel "$LABEL" --missingDataAsZero \
        --outFileName "$outdir/matrix.gz" --outFileSortedRegions "$outdir/sorted_regions.bed"
    plot_matrix "$outdir/matrix.gz" "$outdir" "$prefix" "$title" "$cmap" "$ylabel" \
...
run_enhancer() {
    ...
    computeMatrix reference-point --referencePoint center -b 1000 -a 1000 --binSize 25 -p "$THREADS" \
        -R "$ENHANCERS" -S "$signal" --samplesLabel "$LABEL" --missingDataAsZero \
        --outFileName "$outdir/matrix.gz" --outFileSortedRegions "$outdir/sorted_regions.bed"
    plot_matrix "$outdir/matrix.gz" "$outdir" "$prefix" "$title" "$cmap" "$ylabel" \
        --refPointLabel "Centre" --xAxisLabel "Distance from enhancer centre (bp)"
}
...
run_pmd() {
    ...
    computeMatrix scale-regions -b 10000 -a 10000 --regionBodyLength 5000 --binSize 100 -p "$THREADS" \
        -R "$PMDS" -S "$signal" --samplesLabel "$LABEL" --missingDataAsZero \
        --outFileName "$outdir/matrix.gz" --outFileSortedRegions "$outdir/sorted_regions.bed"
    plot_matrix "$outdir/matrix.gz" "$outdir" "$prefix" "$title" "$cmap" "$ylabel" \
        --startLabel "Start" --endLabel "End" --xAxisLabel "PMD body: 5 kb; flanks: +/-10 kb"
}

for tissue in liver kidney; do
    if [[ "$tissue" == "liver" ]]; then
        LABEL="Liver"
        ENHANCERS="$LIVER_ENHANCERS"; PMDS="$LIVER_PMDS"
        ATAC="$LIVER_ATAC"; WGBS="$LIVER_WGBS"; H3K4ME3="$LIVER_H3K4ME3"
        H3K4ME1="$LIVER_H3K4ME1"; H3K27AC="$LIVER_H3K27AC"; H3K9ME3="$LIVER_H3K9ME3"
    else
        LABEL="Kidney"
        ENHANCERS="$KIDNEY_ENHANCERS"; PMDS="$KIDNEY_PMDS"
        ATAC="$KIDNEY_ATAC"; WGBS="$KIDNEY_WGBS"; H3K4ME3="$KIDNEY_H3K4ME3"
        H3K4ME1="$KIDNEY_H3K4ME1"; H3K27AC="$KIDNEY_H3K27AC"; H3K9ME3="$KIDNEY_H3K9ME3"
    fi
    ...
    run_enhancer "$ATAC" "$OUTROOT/enhancers_ATAC/$tissue" "${tissue}_enhancers_ATAC" "$LABEL ATAC accessibility at enhancers" "magma" "Mean ATAC signal"
    run_enhancer "$WGBS" "$OUTROOT/enhancers_WGBS/$tissue" "${tissue}_enhancers_WGBS" "$LABEL DNA methylation at enhancers" "viridis" "Mean methylation signal"
    run_enhancer "$H3K4ME1" "$OUTROOT/enhancers_H3K4me1/$tissue" "${tissue}_enhancers_H3K4me1" "$LABEL H3K4me1 signal at enhancers" "YlGn" "Mean H3K4me1 signal"
    run_enhancer "$H3K27AC" "$OUTROOT/enhancers_H3K27ac/$tissue" "${tissue}_enhancers_H3K27ac" "$LABEL H3K27ac signal at enhancers" "YlOrBr" "Mean H3K27ac signal"
...
echo "Finished. All tissue-matched results are in: $OUTROOT"
